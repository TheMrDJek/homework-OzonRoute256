...
";
    ...
    private const string InsertTransactionOutboxCommand = 
        @"insert into payment_outbox(id)
        values (@id)";

    private const string DeleteTransactionOutboxCommand =
        @"delete * from payment_outbox
          where id = @id";

    private const string FindByPaymentsQuery =
        @"select * from payment_outbox";

    public PaymentRepository(string connectionString)
        => _connectionString = connectionString;

    public async Task<List<PaymentOutbox>> FindByPayments(CancellationToken cancellationToken)
    {
        await using var connection = new NpgsqlConnection(_connectionString);

        await connection.OpenAsync(cancellationToken);

        var dao = await connection.QueryAsync<PaymentOutbox>(FindByPaymentsQuery);

        return dao.ToList();
    }
    
    public async Task InsertTransactionOutbox(Guid paymentId, CancellationToken cancellationToken)
    {
        await using var connection = new NpgsqlConnection(_connectionString);

        await connection.OpenAsync(cancellationToken);

        await connection.ExecuteAsync(InsertTransactionOutboxCommand, new
        {
            id = paymentId,
            retry = 0
        });
    ...
    public async Task DeleteTransactionOutbox(Guid paymentId, CancellationToken cancellationToken)
    {
        await using var connection = new NpgsqlConnection(_connectionString);

        await connection.OpenAsync(cancellationToken);

        await connection.ExecuteAsync(DeleteTransactionOutboxCommand, new
        {
            id = paymentId,
        });
    }
